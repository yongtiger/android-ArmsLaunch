...
import cc.brainbook.armssplash.splash.util.SharedPreferencesUtil;
 ...
 * 4）处理各个引导页面的返回结果
 */
///注意：启动页面SplashActivity要用Activity而不是AppCompatActivity，后者会自动加载ActionBar，而SplashActivity无需ActionBar
...
//        initLoadGuideInfo();
     ...
     * 初始化过程完成
     *
     * 每完成一个初始化过程就减一，直到为负数表示全部初始化过程都完成了，此时再执行退出SplashActivity、进入MainActivity
     */
